...
func capabilities(fs *flag.FlagSet, args []string) error {
	...
	return json.NewEncoder(os.Stdout).Encode(map[string]any{
		...
		"inputs": capabilityInputs, "results": connection.ResultSchemas(),
		"errors": map[string]any{"encoding": "text/stderr", "exitCode": 1, "prefix": "Burrow: ", "structured": false,
			"semantics": "Validation, ownership, unavailable-resource, approval, transport and persistence failures are text errors. Success/review JSON exits 0. A returned Run or transfer can still contain a failed/partial/uncertain outcome; inspect its state and capture fields. No stable error codes or automatic remote-exit propagation are promised."},
		...
		"presentation": map[string]any{
			...
			"stdout":   "Discovery always emits JSON. Other nonterminal stdout emits JSON; terminal stdout uses the shared semantic renderer. Shell/log/follow routes are interactive. Global flags precede the command; tokens after -- in run commands belong to the target program.",
		},
...
var frontendOperations = []connection.Operation{
	...
	frontendOp("workspace.open", "workspace", "Initialize or reuse the verified workspace", "burrow --workspace PATH status", "New workspace form, or launch burrow --workspace PATH", "supported", "Workspace", "Initializes local workspace/cache as needed, downloads pinned Hovel unless --offline, starts or verifies the daemon, creates default profiles/file roots, and registers the base module. Never connects to an SSH target. status is not read-only startup.", "The explicit invocation authorizes setup; ambiguous or stale owners are refused.", "core/cmd/burrow/main.go", "core/cmd/burrow/setup_check.py"),
	frontendOp("workspace.inspect", "workspace", "Refresh an already-open workspace's daemon health", "No standalone read-only status route; CLI status initializes", "TUI status or Menu → Refresh daemon status", "unsupported", "Workspace", "Inspects launch.Status in an existing frontend; does not initialize. Headless read-only workspace health is a current gap.", "None.", "core/cmd/burrow/frame.go", "core/cmd/burrow/terminal_check.py"),
	...
	frontendOp("workspace.restart", "workspace", "Retire the workspace manager and open the current frontend", "burrow --workspace PATH restart [--yes]", "CLI restart (terminal required)", "terminal-only", "Terminal", "Closes the manager and all its connections/shells, including concurrent additions; preserves saved settings, evidence and Hovel. May initialize workspace setup first.", "Type restart in the terminal, or explicitly supply --yes. Refuses unknown ownership.", "core/cmd/burrow/main.go", "core/cmd/burrow/manager_compatibility_check.py"),
	...
	frontendOp("installation.package", "installation", "Build the local Linux amd64 distributable", "aspect burrow package", "Same Aspect command; extract the produced tgz and run burrow", "supported", "Terminal", "Builds the executable, base-module manifest and upstream license; does not start a workspace. No released distribution installer or automatic Burrow upgrade route.", "Explicit build invocation.", ".aspect/launch.axl", "core/cmd/burrow/setup_check.py"),
	frontendOp("installation.skills", "installation", "Operator skill installation remains follow-on work", "No production skill install/update command", "Historical core/prototype_agent proof only", "unsupported", "Terminal", "No production action. Native client discovery is opt-in; plugin distribution and mandatory MCP are outside this slice.", "Not applicable; unsupported.", "core/prototype_agent/README.md", "core/prototype_agent/README.md"),
}
...
var capabilityInputs = map[string]any{
	...
	"review":             map[string]any{"type": "string", "description": "Optional --review HASH from recap; mandatory for confirmed transfers. Not accepted by close or tunnel remove."},
