...
func capabilities(fs *flag.FlagSet, args []string) error {
	...
	return json.NewEncoder(os.Stdout).Encode(map[string]any{
		...
		"errors": map[string]any{"encoding": "text/stderr", "exitCode": 1, "prefix": "Burrow: ", "structured": false,
			...
			"lifecycle": map[string]any{"routes": []string{"close", "profile connect", "session create|list|inspect|close|claim|takeover|input|resize|release|observe|snapshot"}, "encoding": "JSON/stderr", "exitCode": 1, "shape": "{error:{operation,workspacePath?,code:'operation_failed',message}}", "semantics": "Validation, stale review, ownership, dispatch and cleanup failures are structured. The message distinguishes the refusal; do not automatically retry an uncertain operation. Global flag parsing errors remain text."},
		...
		"integration": map[string]any{
			...
			"daemon":   "Supported public Hovel RPC over the verified owner-protected workspace socket; Burrow-started pinned instances only. Burrow does not publish a separate REST service or SDK. General existing-daemon attachment and generic Mesh routing remain unsupported. Retained shell controls use RunSessionCommand with Request.command and private JSON in Request.inputData (Request.inputEncoding=utf-8), never Args for tokens. Observe uses args:[decimal byte offset]; snapshot optionally accepts args:[history lines 0..1000]; inspect has no arguments. Raw WriteSession refuses and ReadSession/TailSession return no terminal bytes. TUI tabs use these same retained sessions and private controls.",
...
var frontendOperations = []connection.Operation{
	frontendOp("logs.follow", "workspace", "Watch new Hovel and Burrow activity without initializing or controlling work", "burrow --workspace PATH follow [--json]", "Separate terminal; normal TUI and agent CLI can run alongside", "supported", "Activity", "Streams version-1 NDJSON with --json or nonterminal stdout; otherwise uses semantic colors and native scrollback. Starts with current resources and new evidence, not history replay. Includes Hovel PollLogs, existing Burrow audit evidence, retained-run stdout/stderr, transfer progress and shared-shell state/control results. Shell resourceID and connectionID bind the exact owner; id matches the owner-generated control requestID; actor is only a controller label and runID is the shell launch. Hovel supplies no retained-control caller/request identity or generic command-completion event. Submitted input counts and providerResult.acceptedBytes never prove remote command completion. Tokens, keystrokes and terminal bytes are excluded; explicit session observe/snapshot routes remain independent. Each viewer owns volatile cursors; gap/resumed events expose lost history, shell owner failures, incomplete audit and daemon changes. Run output data is base64 with stream byte offsets; output-end marks a captured run stream end. See operation logs documentation for limits.", "No approvals or target actions; requires verified existing daemon. Ctrl+C stops only this viewer; restarting cannot change shell control. Startup errors use text stderr/nonzero exit; recoverable source errors stay in the stream. NO_COLOR/global --no-color preserve text.", "core/cmd/burrow/activity.go", "core/cmd/burrow/activity_check.py"),
	...
	frontendOp("workspace.quit", "workspace", "Review connections across opened workspaces before quitting", "No standalone headless frontend-quit route", "TUI quit / Ctrl+C: keep running, close reviewed connections, or cancel", "unsupported", "Terminal", "Keep releases this frontend's claims and preserves retained shells, connections and tunnels; close tears down only reviewed resources across opened workspaces. Cancel preserves the frontend and claims.", "Shared quit review; uncertain ownership refuses teardown.", "core/cmd/burrow/quit.go", "core/cmd/burrow/connection_lab.py"),
	...
	frontendOp("installation.skills", "installation", "Install or update the bundled operator skills offline", "burrow agent install claude|codex|opencode --scope user|project [--source PATH] [--dry-run]", "CLI; explicit scope, optional dry-run, shared semantic JSON output", "supported", "SkillInstallation", "Installs the bundled discovery, inspection, execution, transfer, tunnel and shared-session skills into the client's documented discovery directory. --source selects an absolute trusted directory extracted from burrow-agent.zip; otherwise uses the embedded release. No network/cache/daemon/client initialization. Preserves edited/unmanaged conflicts and unrelated integrations; updates retain previous directories outside discovery, with per-skill failure recovery. Identical unmanaged content is unchanged, not adopted. No suite transaction, concurrent-editor isolation, plugin distribution, mandatory MCP or model runtime. Omitted skills stay installed.", "Explicit install invocation and required scope; --dry-run preflights the complete suite without writing. Check complete and every skill state; on failed restoration the backup path identifies retained content. Native discovery is opt-in and missing clients are reported.", "core/agent/install.go", "core/cmd/burrow/agent_check.py"),
}
...
var capabilityInputs = map[string]any{
	...
	"history-offset":     map[string]any{"type": "integer", "minimum": 0, "maximum": 1000, "default": 0, "description": "Snapshot history offset in lines, clamped to available history; 0 is current display. Observer-local, no input or geometry change."},
	"shell-id":           map[string]any{"type": "integer", "minimum": 1, "description": "Canonical positive decimal ID in the frontend's shells inventory. shell-close/control/detach default to last selected shell; these numbers can change and are not retained session IDs."},
