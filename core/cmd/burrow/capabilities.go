...
func capabilities(fs *flag.FlagSet, args []string) error {
	...
	return json.NewEncoder(os.Stdout).Encode(map[string]any{
		...
		"errors": map[string]any{"encoding": "text/stderr", "exitCode": 1, "prefix": "Burrow: ", "structured": false,
			...
			"lifecycle": map[string]any{"routes": []string{"close", "profile connect", "session create|list|inspect|close"}, "encoding": "JSON/stderr", "exitCode": 1, "shape": "{error:{operation,workspacePath?,code:'operation_failed',message}}", "semantics": "Validation, stale review, ownership, dispatch and cleanup failures are structured. The message distinguishes the refusal; do not automatically retry an uncertain operation. Global flag parsing errors remain text."},
		...
		"integration": map[string]any{
			...
			"daemon":   "Supported public Hovel RPC over the verified owner-protected workspace socket; Burrow-started pinned instances only. Burrow does not publish a separate REST service or SDK. General existing-daemon attachment, generic Mesh routing and shared shell input/observation remain unsupported. session create/list/inspect/close manages retained shell lifecycle through the existing manager.",
		...
		"presentation": map[string]any{
			...
			"stdout":   "Discovery always emits JSON. Workspace operations, close, profile connect and session commands use structured stderr on failure; human terminals apply the shared semantic JSON renderer to their results and errors. Other nonterminal stdout emits JSON; terminal stdout uses the shared semantic renderer. Shell, logs and run follow routes are interactive. Workspace follow emits NDJSON with --json or a pipe, otherwise a colored scrolling feed. Global flags precede the command; tokens after -- in run commands belong to the target program.",
		},
