...
func capabilities(fs *flag.FlagSet, args []string) error {
	...
	return json.NewEncoder(os.Stdout).Encode(map[string]any{
		...
		"presentation": map[string]any{
			...
			"stdout":   "Discovery always emits JSON. Workspace operations, close and profile connect use structured stderr on failure; human terminals apply the shared semantic JSON renderer to their results and errors. Other nonterminal stdout emits JSON; terminal stdout uses the shared semantic renderer. Shell/log/follow routes are interactive. Global flags precede the command; tokens after -- in run commands belong to the target program.",
		},
