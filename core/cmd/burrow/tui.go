...
func terminal(info launch.Info, noColor bool) error {
	input := textinput.New()
	input.Prompt = "╰─ "
	input.Placeholder = "status · help · quit"
	input.SetSuggestions([]string{"status", "help", "quit"})
...
func (m ui) Init() tea.Cmd { return m.input.Focus() }
func (m ui) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(v, enter):
			...
			default:
				m.output = "Unknown command. Available: status, help, quit."
			}
			return m, nil
		}
	}
...
func (m ui) View() tea.View {
	...
	if m.busy {
		footer = "Verifying… prompt remains editable"
	}
	...
	if m.help || m.quitting {
		...
		pw := max(1, min(64, w-2))
		popup := fit(text, max(1, pw-6), max(1, min(11, h-4)))
		if !m.noColor {
