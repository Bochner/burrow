...
func (Module) Run(ctx *hovel.Context) (hovel.Result, error) {
	...
	if line := ctx.InputString("command", ""); line != "" {
		...
		if len(args) == 0 || (args[0] != "profile" && args[0] != "profiles" && args[0] != "history" && args[0] != "scp" && args[0] != "local" && args[0] != "lcd" && args[0] != "lls" && args[0] != "files-history") || (args[0] == "profile" && len(args) > 1 && args[1] == "connect") {
		...
		if args[0] == "scp" || args[0] == "local" || args[0] == "lcd" || args[0] == "lls" || args[0] == "files-history" {
